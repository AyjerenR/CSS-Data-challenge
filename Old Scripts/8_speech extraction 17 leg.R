...
load("17leg_metadata_speakers_complete_df_encoded.Rdata")
load("17leg_metadata_df.Rdata")
setwd("~/Dropbox (Proksch RA Research)/Proksch RA Research Folder/German Parliamentary Debates/17th legislature txts")
#setwd("~/Dropbox/Research/proksch ra research folder/German Parliamentary Debates/17th legislature txts") #SOP
library(stringr)
...
for (i in 1:nrow(metadata_df17)) {
  ...
  plpr3<-metadata_df17[i, "Plenary_protocol3"] #both 'plpr3' and 'Plenary_protocol3' need to be changed to 'plpr1'/'plpr2' and 'Plenary_protocol1'/'Plenary_protocol2' etc. when changing the reading
  speakers.df <- subset(metadata_speakers_complete_df17, Drucksachennummer==bill & Reading==3 & Type_of_speech=="Rede") #subsetting the dataframe to the (if any) speakers on this bill and this reading -- 'Reading==n' must be changed for each reading
  if (nrow(speakers.df)>0) { #if there any speakes for this reading
    ...
    if (str_detect(metadata_df17[i, "Plenary_protocol3"], ";")) {  #if it has multiple plenary sessions for each reading, remove and save for complicated (budget) loop -- 'Plenary_protocol3' must be changed for each reading
      ...
      if (text.name<=18000) { #actually useless line...kept in from when this loop was intended to run over the 16th-18th legislatures
...
#complicated for first reading:165, 319, 491, 580
#complicated for second reading: 26,28,34,165,319,491,580

##2.
#budget debates - first and second reading
complicated<-c(26,28,34,165,319,491,580) #the subset of complicated (budget debates)
for (i in 1:length(complicated)) {
  i<-complicated[i]
  bill<-metadata_df17[i, "Drucksachennummer"]
  plpr2<-unlist(str_split(metadata_df17[i, "Plenary_protocol2"], ";"))
  for (q in 1:length(plpr1)) { #for each session in each reading
    speakers.df <- subset(metadata_speakers_complete_df17, Drucksachennummer==bill & Reading==2 & Type_of_speech=="Rede" & Comments==paste0("Reading 2: Session:",plpr2[q])) #extracting the information of the speakers in this specific session
