...
##This script works for the 17th and 17th legislature, although some variables need to be manually changed when switching legislatures.
...
load("stable_urls_17.Rdata") #loading vector of stable URLs - must be changed when switching legislatures.
...
metadata_speakers_df17<-data.frame(matrix(ncol = 34))
names(metadata_speakers_df17)<-c("Legislative_period", "Static_URL", "Sachgebiete", "Title", 
...
for (i in 1:length(stable_urls17)) { #'stable_urls17' must be changed when switching legislatures
  ##going to url
  url<-stable_urls17[i] #storing first url
  ...
  if ((length(plpr1)>1)|(length(plpr2)>1)|(length(plpr3)>1)) { #if there are multiple first readings or second readings or third readings etc. it will be dealt with separately
    ...
    if (!(is.null(plpr1)&is.null(plpr2)&is.null(plpr3))) {  #if all th
      ...
      if (!is.null(plpr1)) {
        ...
        if (number_speakers1>0) { #if anyone spoke in the 1st debate
          ...
          for (j in 1:number_speakers1) { #for each of the speakers
            ...
            if (str_detect(speaker_info1[[j]][1, "FUNKTION"], "Bundeskanzl.|BundestagsprÃ¤s.|Bundestagspräs.")) {  #if the speaker is Angela Merkel or Norbert Lammert then the FRAKTION node is missing
              metadata_speakers_df17[j+endrow, "Speaker party"]<-"CDU/CSU"
              ...
              if (str_detect(speaker_info1[[j]][1, "FUNKTION"], "Beauftr. d. Bundesregierung")) {
                metadata_speakers_df17[j+endrow, "Speaker party"]<-NA
              } else { #continue as normal
                metadata_speakers_df17[j+endrow, "Speaker party"]<-paste(speaker_info1[[j]][1, "FRAKTION"], speaker_info1[[j]][1, "RESSORT"], speaker_info1[[j]][1, "FUNKTIONSZUSATZ"])
              }
            }
            metadata_speakers_df17[j+endrow, "Type of speech"]<-speaker_info1[[j]][1, "AKTIVITAETSART"]
            if(!is.null(xpathSApply(parsed_xml, query1.2, xmlValue))){
              if("BESCHLUSSSEITE"%in%names(beschluss1[[1]])){
                metadata_speakers_df17[j+endrow, "Beschlussseite"]<-beschluss1[[1]]$BESCHLUSSSEITE
              }
              if("BESCHLUSSTENOR"%in%names(beschluss1[[1]])){
                metadata_speakers_df17[j+endrow, "Beschlusstenor"]<-beschluss1[[1]]$BESCHLUSSTENOR
              }
              if("BEZUGSDOKUMENT"%in%names(beschluss1[[1]])){
                metadata_speakers_df17[j+endrow, "Beschlussdokument"]<-beschluss1[[1]]$BEZUGSDOKUMENT
              }
              if("ABSTIMMUNGSART"%in%names(beschluss1[[1]])){
                metadata_speakers_df17[j+endrow, "Abstimmungsart"]<-beschluss1[[1]]$ABSTIMMUNGSART
              }
              if("MEHRHEIT"%in%names(beschluss1[[1]])){
                metadata_speakers_df17[j+endrow, "Mehrheit"]<-beschluss1[[1]]$MEHRHEIT
              }
              if("ABSTIMMUNG_BEMERKUNG"%in%names(beschluss1[[1]])){
                metadata_speakers_df17[j+endrow, "Abstimmungsergebnis"]<-beschluss1[[1]]$ABSTIMMUNG_BEMERKUNG
              }
            }
          }
          metadata_speakers_df17[(1+endrow):(endrow + number_speakers1), "Reading"]<-1 #inputting 1 for the reading for all of the rows just initialized by the speakers
          endrow<-nrow(metadata_speakers_df17)
        } else { #if there are no speakers in the 1st debate, make note of it 
          metadata_speakers_df17[endrow+1, "Comments"] <-"No speakers in 1st debate"
          metadata_speakers_df17[endrow+1, "Reading"]<-1
          if(!is.null(xpathSApply(parsed_xml, query1.2, xmlValue))){
            if("BESCHLUSSSEITE"%in%names(beschluss1[[1]])){
              metadata_speakers_df17[1+endrow, "Beschlussseite"]<-beschluss1[[1]]$BESCHLUSSSEITE
            }
            if("BESCHLUSSTENOR"%in%names(beschluss1[[1]])){
              metadata_speakers_df17[1+endrow, "Beschlusstenor"]<-beschluss1[[1]]$BESCHLUSSTENOR
            }
            if("BEZUGSDOKUMENT"%in%names(beschluss1[[1]])){
              metadata_speakers_df17[1+endrow, "Beschlussdokument"]<-beschluss1[[1]]$BEZUGSDOKUMENT
            }
            if("ABSTIMMUNGSART"%in%names(beschluss1[[1]])){
              metadata_speakers_df17[1+endrow, "Abstimmungsart"]<-beschluss1[[1]]$ABSTIMMUNGSART
            }
            if("MEHRHEIT"%in%names(beschluss1[[1]])){
              metadata_speakers_df17[1+endrow, "Mehrheit"]<-beschluss1[[1]]$MEHRHEIT
            }
            if("ABSTIMMUNG_BEMERKUNG"%in%names(beschluss1[[1]])){
              metadata_speakers_df17[1+endrow, "Abstimmungsergebnis"]<-beschluss1[[1]]$ABSTIMMUNG_BEMERKUNG
            }
          }
          endrow<-nrow(metadata_speakers_df17)
        ...
        metadata_speakers_df17[endrow+1, "Comments"]<-"No 1st debate"
        metadata_speakers_df17[endrow+1, "Date_of_BT_debate1"]<-"No 1st debate"
        metadata_speakers_df17[endrow+1, "Reading"]<-1
        endrow<-nrow(metadata_speakers_df17)
        number_speakers1<-1 #storing the number 1 so I know there is 1 row that needs to be filled with bill information
      }
      ...
      if (!is.null(plpr2)) {
        ...
        if (number_speakers2>0) {
          ...
          for (k in 1:number_speakers2) {
            #metadata_speakers_df17[k+endrow,"Speaker name"]<-str_trim(paste(speaker_info2[[k]][1,"PERSON_TITEL"], speaker_info2[[k]][1,"VORNAME"], speaker_info2[[k]][1,"NAMENSZUSATZ"], speaker_info2[[k]][1,"NACHNAME"], speaker_info2[[k]][1,"WAHLKREISZUSATZ"])) #next idea is to subset within the list and then just paste into the dataframe
            metadata_speakers_df17[k+endrow,"Speaker name"]<-paste(speaker_info2[[k]][1,"PERSON_TITEL"], speaker_info2[[k]][1,"VORNAME"], speaker_info2[[k]][1,"NAMENSZUSATZ"], speaker_info2[[k]][1,"NACHNAME"], speaker_info2[[k]][1,"WAHLKREISZUSATZ"]) #next idea is to subset within the list and then just paste into the dataframe
            metadata_speakers_df17[k+endrow, "Speaker type"]<-speaker_info2[[k]][1, "FUNKTION"]
            if (str_detect(speaker_info2[[k]][1, "FUNKTION"], "Bundeskanzl.|BundestagsprÃ¤s.")) {  #if the speaker is Angela Merkel or Norbert Lammert then the FRAKTION node is missing
              metadata_speakers_df17[k+endrow, "Speaker party"]<-"CDU/CSU"
            } else { #if it's not Merkel or Lammert, continue as normal..unless it's the 'Beauftr. d. Bundesregierung'
              if (str_detect(speaker_info2[[k]][1, "FUNKTION"], "Beauftr. d. Bundesregierung")) {
                metadata_speakers_df17[k+endrow, "Speaker party"]<-NA
              } else { #continue as normal
                #metadata_speakers_df17[k+endrow, "Speaker party"]<-str_trim(paste(speaker_info2[[k]][1, "FRAKTION"], speaker_info2[[k]][1, "RESSORT"], speaker_info2[[k]][1, "FUNKTIONSZUSATZ"]))
                metadata_speakers_df17[k+endrow, "Speaker party"]<-paste(speaker_info2[[k]][1, "FRAKTION"], speaker_info2[[k]][1, "RESSORT"], speaker_info2[[k]][1, "FUNKTIONSZUSATZ"])
                
              }
            }
            metadata_speakers_df17[k+endrow, "Type of speech"]<-speaker_info2[[k]][1, "AKTIVITAETSART"]
            if(!is.null(xpathSApply(parsed_xml, query2.2, xmlValue))){
              if("BESCHLUSSSEITE"%in%names(beschluss2[[1]])){
                metadata_speakers_df17[k+endrow, "Beschlussseite"]<-beschluss2[[1]]$BESCHLUSSSEITE
              }
              if("BESCHLUSSTENOR"%in%names(beschluss2[[1]])){
                metadata_speakers_df17[k+endrow, "Beschlusstenor"]<-beschluss2[[1]]$BESCHLUSSTENOR
              }
              if("BEZUGSDOKUMENT"%in%names(beschluss2[[1]])){
                metadata_speakers_df17[k+endrow, "Beschlussdokument"]<-beschluss2[[1]]$BEZUGSDOKUMENT
              }
              if("ABSTIMMUNGSART"%in%names(beschluss2[[1]])){
                metadata_speakers_df17[k+endrow, "Abstimmungsart"]<-beschluss2[[1]]$ABSTIMMUNGSART
              }
              if("MEHRHEIT"%in%names(beschluss2[[1]])){
                metadata_speakers_df17[k+endrow, "Mehrheit"]<-beschluss2[[1]]$MEHRHEIT
              }
              if("ABSTIMMUNG_BEMERKUNG"%in%names(beschluss2[[1]])){
                metadata_speakers_df17[k+endrow, "Abstimmungsergebnis"]<-beschluss2[[1]]$ABSTIMMUNG_BEMERKUNG
              }
            }
          }
          metadata_speakers_df17[(1+endrow):(endrow + number_speakers2), "Reading"]<-2
          endrow<-nrow(metadata_speakers_df17)
        } else { #if there are no speakers in the 2nd debate, make note of it 
          metadata_speakers_df17[endrow+1, "Comments"]<-"No speakers in 2nd debate"
          metadata_speakers_df17[endrow+1, "Reading"]<-2
          if(!is.null(xpathSApply(parsed_xml, query2.2, xmlValue))){
            if("BESCHLUSSSEITE"%in%names(beschluss2[[1]])){
              metadata_speakers_df17[1+endrow, "Beschlussseite"]<-beschluss2[[1]]$BESCHLUSSSEITE
            }
            if("BESCHLUSSTENOR"%in%names(beschluss2[[1]])){
              metadata_speakers_df17[1+endrow, "Beschlusstenor"]<-beschluss2[[1]]$BESCHLUSSTENOR
            }
            if("BEZUGSDOKUMENT"%in%names(beschluss2[[1]])){
              metadata_speakers_df17[1+endrow, "Beschlussdokument"]<-beschluss2[[1]]$BEZUGSDOKUMENT
            }
            if("ABSTIMMUNGSART"%in%names(beschluss2[[1]])){
              metadata_speakers_df17[1+endrow, "Abstimmungsart"]<-beschluss2[[1]]$ABSTIMMUNGSART
            }
            if("MEHRHEIT"%in%names(beschluss2[[1]])){
              metadata_speakers_df17[1+endrow, "Mehrheit"]<-beschluss2[[1]]$MEHRHEIT
            }
            if("ABSTIMMUNG_BEMERKUNG"%in%names(beschluss2[[1]])){
              metadata_speakers_df17[1+endrow, "Abstimmungsergebnis"]<-beschluss2[[1]]$ABSTIMMUNG_BEMERKUNG
            }
          }
          endrow<-nrow(metadata_speakers_df17)
        ...
        metadata_speakers_df17[endrow+1, "Comments"]<-"No 2nd debate"
        metadata_speakers_df17[endrow+1, "Date_of_BT_debate2"]<-"No 2nd debate"
        metadata_speakers_df17[endrow+1, "Reading"]<-2
        endrow<-nrow(metadata_speakers_df17)
        number_speakers2<-1 #storing the number 1 so I know there is 1 row that needs to be filled with bill information
      }
      ...
      if (!is.null(plpr3)) {
        ...
        if (number_speakers3>0){
          ...
          for (m in 1:number_speakers3) {
            #metadata_speakers_df17[m+endrow, "Speaker name"]<-str_trim(paste(speaker_info3[[m]][1,"PERSON_TITEL"], speaker_info3[[m]][1,"VORNAME"], speaker_info3[[m]][1,"NAMENSZUSATZ"], speaker_info3[[m]][1,"NACHNAME"], speaker_info3[[m]][1,"WAHLKREISZUSATZ"])) #next idea is to subset within the list and then just paste into the dataframe
            metadata_speakers_df17[m+endrow,"Speaker name"]<-paste(speaker_info3[[m]][1,"PERSON_TITEL"], speaker_info3[[m]][1,"VORNAME"], speaker_info3[[m]][1,"NAMENSZUSATZ"], speaker_info3[[m]][1,"NACHNAME"], speaker_info3[[m]][1,"WAHLKREISZUSATZ"]) #next idea is to subset within the list and then just paste into the dataframe
            metadata_speakers_df17[m+endrow, "Speaker type"]<-speaker_info3[[m]][1, "FUNKTION"]
            if (str_detect(speaker_info3[[m]][1, "FUNKTION"], "Bundeskanzl.|BundestagsprÃ¤s.")) {  #if the speaker is Angela Merkel or Norbert Lammert then the FRAKTION node is missing
              metadata_speakers_df17[m+endrow, "Speaker party"]<-"CDU/CSU"
            } else { #if it's not Merkel or Lammert, continue as normal..unless it's the 'Beauftr. d. Bundesregierung'
              if (str_detect(speaker_info3[[m]][1, "FUNKTION"], "Beauftr. d. Bundesregierung")) {
                metadata_speakers_df17[m+endrow, "Speaker party"]<-NA
              } else { #continue as normal
                #metadata_speakers_df17[m+endrow, "Speaker party"]<-str_trim(paste(speaker_info3[[m]][1, "FRAKTION"], speaker_info3[[m]][1, "RESSORT"], speaker_info3[[m]][1, "FUNKTIONSZUSATZ"]))
                metadata_speakers_df17[m+endrow, "Speaker party"]<-paste(speaker_info3[[m]][1, "FRAKTION"], speaker_info3[[m]][1, "RESSORT"], speaker_info3[[m]][1, "FUNKTIONSZUSATZ"])
                
              }
            }
            metadata_speakers_df17[m+endrow, "Type of speech"]<-speaker_info3[[m]][1, "AKTIVITAETSART"]
            if(!is.null(xpathSApply(parsed_xml, query3.2, xmlValue))){
              if("BESCHLUSSSEITE"%in%names(beschluss3[[1]])){
                metadata_speakers_df17[m+endrow, "Beschlussseite"]<-beschluss3[[1]]$BESCHLUSSSEITE
              }
              if("BESCHLUSSTENOR"%in%names(beschluss3[[1]])){
                metadata_speakers_df17[m+endrow, "Beschlusstenor"]<-beschluss3[[1]]$BESCHLUSSTENOR
              }
              if("BEZUGSDOKUMENT"%in%names(beschluss3[[1]])){
                metadata_speakers_df17[m+endrow, "Beschlussdokument"]<-beschluss3[[1]]$BEZUGSDOKUMENT
              }
              if("ABSTIMMUNGSART"%in%names(beschluss3[[1]])){
                metadata_speakers_df17[m+endrow, "Abstimmungsart"]<-beschluss3[[1]]$ABSTIMMUNGSART
              }
              if("MEHRHEIT"%in%names(beschluss3[[1]])){
                metadata_speakers_df17[m+endrow, "Mehrheit"]<-beschluss3[[1]]$MEHRHEIT
              }
              if("ABSTIMMUNG_BEMERKUNG"%in%names(beschluss3[[1]])){
                metadata_speakers_df17[m+endrow, "Abstimmungsergebnis"]<-beschluss3[[1]]$ABSTIMMUNG_BEMERKUNG
              }
            }
          }
          metadata_speakers_df17[(1+endrow):(endrow + number_speakers3), "Reading"]<-3
          endrow<-nrow(metadata_speakers_df17)
        } else { #if there are no speakers in the 3rd debate, make note of it 
          metadata_speakers_df17[endrow+1, "Comments"]<-"No speakers in 3rd debate"
          metadata_speakers_df17[endrow+1, "Reading"]<-3
          if(!is.null(xpathSApply(parsed_xml, query3.2, xmlValue))){
            if("BESCHLUSSSEITE"%in%names(beschluss3[[1]])){
              metadata_speakers_df17[1+endrow, "Beschlussseite"]<-beschluss3[[1]]$BESCHLUSSSEITE
            }
            if("BESCHLUSSTENOR"%in%names(beschluss3[[1]])){
              metadata_speakers_df17[1+endrow, "Beschlusstenor"]<-beschluss3[[1]]$BESCHLUSSTENOR
            }
            if("BEZUGSDOKUMENT"%in%names(beschluss3[[1]])){
              metadata_speakers_df17[1+endrow, "Beschlussdokument"]<-beschluss3[[1]]$BEZUGSDOKUMENT
            }
            if("ABSTIMMUNGSART"%in%names(beschluss3[[1]])){
              metadata_speakers_df17[1+endrow, "Abstimmungsart"]<-beschluss3[[1]]$ABSTIMMUNGSART
            }
            if("MEHRHEIT"%in%names(beschluss3[[1]])){
              metadata_speakers_df17[1+endrow, "Mehrheit"]<-beschluss3[[1]]$MEHRHEIT
            }
            if("ABSTIMMUNG_BEMERKUNG"%in%names(beschluss3[[1]])){
              metadata_speakers_df17[1+endrow, "Abstimmungsergebnis"]<-beschluss3[[1]]$ABSTIMMUNG_BEMERKUNG
            }
          }
          endrow<-nrow(metadata_speakers_df17)
        ...
        metadata_speakers_df17[endrow+1, "Comments"]<-"No 3rd debate"
        metadata_speakers_df17[endrow+1, "Date_of_BT_debate3"]<-"No 3rd debate"
        metadata_speakers_df17[endrow+1, "Reading"]<-3
        endrow<-nrow(metadata_speakers_df17)
        number_speakers3<-1 #storing the number 1 so I know there is 1 row that needs to be filled with bill information
      } 
      ...
      metadata_speakers_df17[(start:endrow),"Legislative_period"]<-wahl
      metadata_speakers_df17[(start:endrow), "Static_URL"]<-url
      metadata_speakers_df17[(start:endrow),"Sachgebiete"]<-sach_collapsed
      metadata_speakers_df17[(start:endrow),"Title"]<-title
      metadata_speakers_df17[(start:endrow),"Inhalt"]<-inhalt
      metadata_speakers_df17[(start:endrow),"Initiator"]<-initiative_collapsed
      metadata_speakers_df17[(start:endrow),"Gesta_number"]<-gesta
      metadata_speakers_df17[(start:endrow),"Aktueller_Stand"]<-akt
      if (length(zust_proc)>0){
        if (str_detect(zust_proc, "Ja")) {metadata_speakers_df17[(start:endrow),"Zustimmungspflicht_proclamation"]<-"yes"}
        if (str_detect(zust_proc, "Nein")) {metadata_speakers_df17[(start:endrow),"Zustimmungspflicht_proclamation"]<-"no"}
      }
      if (length(zust_bill)>0) {
        if (str_detect(zust_bill, "Ja")) {metadata_speakers_df17[(start:endrow),"Zustimmungspflicht_bill"]<-"yes"}
        if (str_detect(zust_bill, "Nein")) {metadata_speakers_df17[(start:endrow),"Zustimmungspflicht_bill"]<-"no"}
      }
      if (length(drs_num)>0) { #if it exists, extract it
        metadata_speakers_df17[(start:endrow),"Drucksachennummer"]<-drs_num  
      }
      metadata_speakers_df17[(start:endrow),"Date_of_initiative"]<-init_date_value
      if (length(ver_dates)>0){
        ver_date_value<-str_extract(ver_dates, "[[:digit:]]([[:digit:]]|\\.){9}") #extracting the first date in this row
        metadata_speakers_df17[(start:endrow),"Proclamation_date"]<-ver_date_value
      }
      if (length(inkraft)>0){
        ...
        metadata_speakers_df17[(start:endrow),"Date_of_entry_into_force"]<-inkraft_value
      }
      #Plenary protocol and reading
      if (!is.null(plpr1)) { #if the protocol is not null, there will be a readind date associated 
        metadata_speakers_df17[(start:endrow), "Plenary_protocol1"]<-plpr1
        ...
        metadata_speakers_df17[(start:endrow), "Date_of_BT_debate1"]<-debate1_date
      }
      if (!is.null(plpr2)) {
        metadata_speakers_df17[(start:endrow), "Plenary_protocol2"]<-plpr2
        ...
        metadata_speakers_df17[(start:endrow), "Date_of_BT_debate2"]<-debate2_date
      }
      if (!is.null(plpr3)) {
        metadata_speakers_df17[(start:endrow), "Plenary_protocol3"]<-plpr3
        ...
        metadata_speakers_df17[(start:endrow), "Date_of_BT_debate3"]<-debate3_date
      }
      endrow<-nrow(metadata_speakers_df17)
    } else {
      #filling in bill variables if there was no debate in the BT
      metadata_speakers_df17[endrow+1,"Legislative_period"]<-wahl
      metadata_speakers_df17[endrow+1, "Static_URL"]<-url
      metadata_speakers_df17[endrow+1,"Sachgebiete"]<-sach_collapsed
      metadata_speakers_df17[endrow+1,"Title"]<-title
      metadata_speakers_df17[endrow+1,"Inhalt"]<-inhalt
      metadata_speakers_df17[endrow+1,"Initiator"]<-initiative_collapsed
      metadata_speakers_df17[endrow+1,"Aktueller_Stand"]<-akt
      metadata_speakers_df17[endrow+1,"Gesta_number"]<-gesta
      if (length(zust_proc)>0){
        if (str_detect(zust_proc, "Ja")) {metadata_speakers_df17[endrow+1,"Zustimmungspflicht_proclamation"]<-"yes"}
        if (str_detect(zust_proc, "Nein")) {metadata_speakers_df17[endrow+1,"Zustimmungspflicht_proclamation"]<-"no"}
      }
      if (length(zust_bill)>0) {
        if (str_detect(zust_bill, "Ja")) {metadata_speakers_df17[endrow+1,"Zustimmungspflicht_bill"]<-"yes"}
        if (str_detect(zust_bill, "Nein")) {metadata_speakers_df17[endrow+1,"Zustimmungspflicht_bill"]<-"no"}
      }
      if (length(drs_num)>0) { #if it exists, extract it
        metadata_speakers_df17[endrow+1,"Drucksachennummer"]<-drs_num  
      }
      metadata_speakers_df17[endrow+1,"Date_of_initiative"]<-init_date_value
      if (length(ver_dates)>0){
        ver_date_value<-str_extract(ver_dates, "[[:digit:]]([[:digit:]]|\\.){9}") #extracting the first date in this row
        metadata_speakers_df17[endrow+1,"Proclamation_date"]<-ver_date_value
      }
      if (length(inkraft)>0){
        inkraft<-inkraft[1] #selecting the first entry
        inkraft_value<-str_sub(inkraft, 1,10) #the date is the first 10 characters of the row
        metadata_speakers_df17[endrow+1,"Date_of_entry_into_force"]<-inkraft_value
      }
      metadata_speakers_df17[endrow+1, "Comments"]<-"No debates"
      metadata_speakers_df17[endrow+1, "Date_of_BT_debate1"]<-"No 1st debate"
      metadata_speakers_df17[endrow+1, "Date_of_BT_debate2"]<-"No 2nd debate"
      metadata_speakers_df17[endrow+1, "Date_of_BT_debate3"]<-"No 3rd debate"
    }
  ...
  print(i/length(stable_urls17))
}

save(metadata_speakers_df17, file="17leg_metadata_speakers_df_encoded.Rdata")
...
metadata_speakers_comp_df17<-data.frame(matrix(ncol = 34))
names(metadata_speakers_comp_df17)<-c("Legislative_period", "Static_URL", "Sachgebiete", "Title", 
                                      "Inhalt","EU_impulse_relationship", "Initiator", "Aktueller_Stand", 
                                      "Gesta_number", "Zustimmungspflicht_proclamation", "Zustimmungspflicht_bill", 
                                      "Drucksachennummer", "Date_of_initiative",  "Proclamation_date", 
                                      "Date_of_entry_into_force", "Date_of_BT_debate1", "Date_of_BT_debate2", 
...
for (n in 1:length(complicated)) {
  ##going to url
  url<-stable_urls17[complicated[n]] #storing the url of the relevant row
  ...
  if (!is.null(plpr1)) {
    for (p in 1:length(plpr1)) { #for each session of first reading
      ...
      if (length(positive_children)>0) { #if there were any children (speakers)
        ...
        for (q in 1:length(query1b)) {
          metadata_speakers_comp_df17[q+endrow,"Speaker name"]<-paste(speaker_info1[[q]][1,"PERSON_TITEL"], speaker_info1[[q]][1,"VORNAME"], speaker_info1[[q]][1,"NAMENSZUSATZ"], speaker_info1[[q]][1,"NACHNAME"], speaker_info1[[q]][1,"WAHLKREISZUSATZ"]) #next idea is to subset within the list and then just paste into the dataframe
          metadata_speakers_comp_df17[q+endrow, "Speaker type"]<-speaker_info1[[q]][1, "FUNKTION"]
          if (str_detect(speaker_info1[[q]][1, "FUNKTION"], "Bundeskanzl.|BundestagsprÃ¤s.")) {  #if the speaker is Angela Merkel or Norbert Lammert then the FRAKTION node is missing
            metadata_speakers_comp_df17[q+endrow, "Speaker party"]<-"CDU/CSU"
          } else { #if it's not Merkel or Lammert, continue as normal
            metadata_speakers_comp_df17[q+endrow, "Speaker party"]<-paste(speaker_info1[[q]][1, "FRAKTION"], speaker_info1[[q]][1, "RESSORT"], speaker_info1[[q]][1, "FUNKTIONSZUSATZ"])
          }
          metadata_speakers_comp_df17[q+endrow, "Type of speech"]<-speaker_info1[[q]][1, "AKTIVITAETSART"]
          if(!is.null(xpathSApply(parsed_xml, query1.2a, xmlValue))){
            if("BESCHLUSS"%in%names(beschluss1[[length(beschluss1)]])){
              metadata_speakers_comp_df17[q+endrow, "Beschluss"]<-beschluss1[[length(beschluss1)]]$BESCHLUSS
            }
            if("BESCHLUSSSEITE"%in%names(beschluss1[[length(beschluss1)]])){
              metadata_speakers_comp_df17[q+endrow, "Beschlussseite"]<-beschluss1[[length(beschluss1)]]$BESCHLUSSSEITE
            }
            if("BESCHLUSSTENOR"%in%names(beschluss1[[length(beschluss1)]])){
              metadata_speakers_comp_df17[q+endrow, "Beschlusstenor"]<-beschluss1[[length(beschluss1)]]$BESCHLUSSTENOR
            }
            if("BEZUGSDOKUMENT"%in%names(beschluss1[[length(beschluss1)]])){
              metadata_speakers_comp_df17[q+endrow, "Beschlussdokument"]<-beschluss1[[length(beschluss1)]]$BEZUGSDOKUMENT
            }
            if("ABSTIMMUNGSART"%in%names(beschluss1[[length(beschluss1)]])){
              metadata_speakers_comp_df17[q+endrow, "Abstimmungsart"]<-beschluss1[[length(beschluss1)]]$ABSTIMMUNGSART
            }
            if("MEHRHEIT"%in%names(beschluss1[[length(beschluss1)]])){
              metadata_speakers_comp_df17[q+endrow, "Mehrheit"]<-beschluss1[[length(beschluss1)]]$MEHRHEIT
            }
            if("ABSTIMMUNG_BEMERKUNG"%in%names(beschluss1[[length(beschluss1)]])){
              metadata_speakers_comp_df17[q+endrow, "Abstimmungsergebnis"]<-beschluss1[[length(beschluss1)]]$ABSTIMMUNG_BEMERKUNG
            }
          }
        }
        metadata_speakers_comp_df17[(1+endrow):(endrow + length(query1b)), "Reading"]<-1 #inputting the relevant information into all the rows initialized by speakers
        metadata_speakers_comp_df17[(1+endrow):(endrow + length(query1b)), "Comments"] <-paste0("Reading 1: Session:", plpr1[p])
        endrow<-nrow(metadata_speakers_comp_df17)
        total_speakers1<-total_speakers1+length(query1b)
      } else { #if there were no speakers in this session, make a note of it
        metadata_speakers_comp_df17[endrow+1, "Comments"] <-paste0("No speakers in Reading 1: Session:", plpr1[p])
        metadata_speakers_comp_df17[endrow+1, "Reading"]<-1
        if(!is.null(xpathSApply(parsed_xml, query1.2a, xmlValue))){
          if("BESCHLUSS"%in%names(beschluss1[[length(beschluss1)]])){
            metadata_speakers_comp_df17[1+endrow, "Beschluss"]<-beschluss1[[length(beschluss1)]]$BESCHLUSS
          }
          if("BESCHLUSSSEITE"%in%names(beschluss1[[length(beschluss1)]])){
            metadata_speakers_comp_df17[1+endrow, "Beschlussseite"]<-beschluss1[[length(beschluss1)]]$BESCHLUSSSEITE
          }
          if("BESCHLUSSTENOR"%in%names(beschluss1[[length(beschluss1)]])){
            metadata_speakers_comp_df17[1+endrow, "Beschlusstenor"]<-beschluss1[[length(beschluss1)]]$BESCHLUSSTENOR
          }
          if("BEZUGSDOKUMENT"%in%names(beschluss1[[length(beschluss1)]])){
            metadata_speakers_comp_df17[1+endrow, "Beschlussdokument"]<-beschluss1[[length(beschluss1)]]$BEZUGSDOKUMENT
          }
          if("ABSTIMMUNGSART"%in%names(beschluss1[[length(beschluss1)]])){
            metadata_speakers_comp_df17[1+endrow, "Abstimmungsart"]<-beschluss1[[length(beschluss1)]]$ABSTIMMUNGSART
          }
          if("MEHRHEIT"%in%names(beschluss1[[length(beschluss1)]])){
            metadata_speakers_comp_df17[1+endrow, "Mehrheit"]<-beschluss1[[length(beschluss1)]]$MEHRHEIT
          }
          if("ABSTIMMUNG_BEMERKUNG"%in%names(beschluss1[[length(beschluss1)]])){
            metadata_speakers_comp_df17[1+endrow, "Abstimmungsergebnis"]<-beschluss1[[length(beschluss1)]]$ABSTIMMUNG_BEMERKUNG
          }
        }
        endrow<-nrow(metadata_speakers_comp_df17)
    ...
    metadata_speakers_comp_df17[endrow+1, "Comments"]<-"No 1st debate"
    metadata_speakers_comp_df17[endrow+1, "Date_of_BT_debate1"]<-"No 1st debate"
    metadata_speakers_comp_df17[endrow+1, "Reading"]<-1
    endrow<-nrow(metadata_speakers_comp_df17)
  ...
  if (!is.null(plpr2)) {
    for (r in 1:length(plpr2)) { #for each session of first reading
      ...
      if (length(positive_children2)>0) {
        ...
        for (s in 1:length(query2b)) {
          metadata_speakers_comp_df17[s+endrow,"Speaker name"]<-paste(speaker_info2[[s]][1,"PERSON_TITEL"], speaker_info2[[s]][1,"VORNAME"], speaker_info2[[s]][1,"NAMENSZUSATZ"], speaker_info2[[s]][1,"NACHNAME"], speaker_info2[[s]][1,"WAHLKREISZUSATZ"]) #next idea is to subset within the list and then just paste into the dataframe
          metadata_speakers_comp_df17[s+endrow, "Speaker type"]<-speaker_info2[[s]][1, "FUNKTION"]
          if (str_detect(speaker_info2[[s]][1, "FUNKTION"], "Bundeskanzl.|BundestagsprÃ¤s.")) {  #if the speaker is Angela Merkel or Norbert Lammert then the FRAKTION node is missing
            metadata_speakers_comp_df17[s+endrow, "Speaker party"]<-"CDU/CSU"
          } else { #if it's not Merkel or Lammert, continue as normal
            metadata_speakers_comp_df17[s+endrow, "Speaker party"]<-paste(speaker_info2[[s]][1, "FRAKTION"], speaker_info2[[s]][1, "RESSORT"], speaker_info2[[s]][1, "FUNKTIONSZUSATZ"])
          }
          metadata_speakers_comp_df17[s+endrow, "Type of speech"]<-speaker_info2[[s]][1, "AKTIVITAETSART"]
          if(!is.null(xpathSApply(parsed_xml, query2.2a, xmlValue))){
            if("BESCHLUSS"%in%names(beschluss2[[length(beschluss2)]])){
              metadata_speakers_comp_df17[s+endrow, "Beschluss"]<-beschluss2[[length(beschluss2)]]$BESCHLUSS
            }
            if("BESCHLUSSSEITE"%in%names(beschluss2[[length(beschluss2)]])){
              metadata_speakers_comp_df17[s+endrow, "Beschlussseite"]<-beschluss2[[length(beschluss2)]]$BESCHLUSSSEITE
            }
            if("BESCHLUSSTENOR"%in%names(beschluss2[[length(beschluss2)]])){
              metadata_speakers_comp_df17[s+endrow, "Beschlusstenor"]<-beschluss2[[length(beschluss2)]]$BESCHLUSSTENOR
            }
            if("BEZUGSDOKUMENT"%in%names(beschluss2[[length(beschluss2)]])){
              metadata_speakers_comp_df17[s+endrow, "Beschlussdokument"]<-beschluss2[[length(beschluss2)]]$BEZUGSDOKUMENT
            }
            if("ABSTIMMUNGSART"%in%names(beschluss2[[length(beschluss2)]])){
              metadata_speakers_comp_df17[s+endrow, "Abstimmungsart"]<-beschluss2[[length(beschluss2)]]$ABSTIMMUNGSART
            }
            if("MEHRHEIT"%in%names(beschluss2[[length(beschluss2)]])){
              metadata_speakers_comp_df17[s+endrow, "Mehrheit"]<-beschluss2[[length(beschluss2)]]$MEHRHEIT
            }
            if("ABSTIMMUNG_BEMERKUNG"%in%names(beschluss2[[length(beschluss2)]])){
              metadata_speakers_comp_df17[s+endrow, "Abstimmungsergebnis"]<-beschluss2[[length(beschluss2)]]$ABSTIMMUNG_BEMERKUNG
            }
          }
        }
        metadata_speakers_comp_df17[(1+endrow):(endrow + length(query2b)), "Reading"]<-2
        metadata_speakers_comp_df17[(1+endrow):(endrow + length(query2b)), "Comments"] <-paste0("Reading 2: Session:", plpr2[r])
        endrow<-nrow(metadata_speakers_comp_df17)
        total_speakers2<-total_speakers2+length(query2b)
      } else {
        metadata_speakers_comp_df17[endrow+1, "Comments"] <-paste0("No speakers in Reading 2: Session:", plpr2[r])
        metadata_speakers_comp_df17[endrow+1, "Reading"]<-2
        if(!is.null(xpathSApply(parsed_xml, query2.2a, xmlValue))){
          if("BESCHLUSS"%in%names(beschluss2[[length(beschluss2)]])){
            metadata_speakers_comp_df17[1+endrow, "Beschluss"]<-beschluss2[[length(beschluss2)]]$BESCHLUSS
          }
          if("BESCHLUSSSEITE"%in%names(beschluss2[[length(beschluss2)]])){
            metadata_speakers_comp_df17[1+endrow, "Beschlussseite"]<-beschluss2[[length(beschluss2)]]$BESCHLUSSSEITE
          }
          if("BESCHLUSSTENOR"%in%names(beschluss2[[length(beschluss2)]])){
            metadata_speakers_comp_df17[1+endrow, "Beschlusstenor"]<-beschluss2[[length(beschluss2)]]$BESCHLUSSTENOR
          }
          if("BEZUGSDOKUMENT"%in%names(beschluss2[[length(beschluss2)]])){
            metadata_speakers_comp_df17[1+endrow, "Beschlussdokument"]<-beschluss2[[length(beschluss2)]]$BEZUGSDOKUMENT
          }
          if("ABSTIMMUNGSART"%in%names(beschluss2[[length(beschluss2)]])){
            metadata_speakers_comp_df17[1+endrow, "Abstimmungsart"]<-beschluss2[[length(beschluss2)]]$ABSTIMMUNGSART
          }
          if("MEHRHEIT"%in%names(beschluss2[[length(beschluss2)]])){
            metadata_speakers_comp_df17[1+endrow, "Mehrheit"]<-beschluss2[[length(beschluss2)]]$MEHRHEIT
          }
          if("ABSTIMMUNG_BEMERKUNG"%in%names(beschluss2[[length(beschluss2)]])){
            metadata_speakers_comp_df17[1+endrow, "Abstimmungsergebnis"]<-beschluss2[[length(beschluss2)]]$ABSTIMMUNG_BEMERKUNG
          }
        }
        endrow<-nrow(metadata_speakers_comp_df17)
    ...
    metadata_speakers_comp_df17[endrow+1, "Comments"]<-"No 2nd debate"
    metadata_speakers_comp_df17[endrow+1, "Date_of_BT_debate2"]<-"No 2nd debate"
    metadata_speakers_comp_df17[endrow+1, "Reading"]<-2
    endrow<-nrow(metadata_speakers_comp_df17)
  ...
  if (!is.null(plpr3)) {
    for (t in 1:length(plpr3)) { #for each session of first reading
      ...
      if (length(positive_children3)>0) {
        ...
        for (u in 1:length(query3b)) {
          metadata_speakers_comp_df17[u+endrow,"Speaker name"]<-paste(speaker_info3[[u]][1,"PERSON_TITEL"], speaker_info3[[u]][1,"VORNAME"], speaker_info3[[u]][1,"NAMENSZUSATZ"], speaker_info3[[u]][1,"NACHNAME"], speaker_info3[[u]][1,"WAHLKREISZUSATZ"]) #next idea is to subset within the list and then just paste into the dataframe
          metadata_speakers_comp_df17[u+endrow, "Speaker type"]<-speaker_info3[[u]][1, "FUNKTION"]
          if (str_detect(speaker_info3[[u]][1, "FUNKTION"], "Bundeskanzl.|BundestagsprÃ¤s.")) {  #if the speaker is Angela Merkel or Norbert Lammert then the FRAKTION node is missing
            metadata_speakers_comp_df17[u+endrow, "Speaker party"]<-"CDU/CSU"
          } else { #if it's not Merkel or Lammert, continue as normal
            metadata_speakers_comp_df17[u+endrow, "Speaker party"]<-paste(speaker_info3[[u]][1, "FRAKTION"], speaker_info3[[u]][1, "RESSORT"], speaker_info3[[u]][1, "FUNKTIONSZUSATZ"])
          }
          metadata_speakers_comp_df17[u+endrow, "Type of speech"]<-speaker_info3[[u]][1, "AKTIVITAETSART"]
          if(!is.null(xpathSApply(parsed_xml, query3.2a, xmlValue))){
            if("BESCHLUSS"%in%names(beschluss3[[length(beschluss3)]])){
              metadata_speakers_comp_df17[u+endrow, "Beschluss"]<-beschluss3[[length(beschluss3)]]$BESCHLUSS
            }
            if("BESCHLUSSSEITE"%in%names(beschluss3[[length(beschluss3)]])){
              metadata_speakers_comp_df17[u+endrow, "Beschlussseite"]<-beschluss3[[length(beschluss3)]]$BESCHLUSSSEITE
            }
            if("BESCHLUSSTENOR"%in%names(beschluss3[[length(beschluss3)]])){
              metadata_speakers_comp_df17[u+endrow, "Beschlusstenor"]<-beschluss3[[length(beschluss3)]]$BESCHLUSSTENOR
            }
            if("BEZUGSDOKUMENT"%in%names(beschluss3[[length(beschluss3)]])){
              metadata_speakers_comp_df17[u+endrow, "Beschlussdokument"]<-beschluss3[[length(beschluss3)]]$BEZUGSDOKUMENT
            }
            if("ABSTIMMUNGSART"%in%names(beschluss3[[length(beschluss3)]])){
              metadata_speakers_comp_df17[u+endrow, "Abstimmungsart"]<-beschluss3[[length(beschluss3)]]$ABSTIMMUNGSART
            }
            if("MEHRHEIT"%in%names(beschluss3[[length(beschluss3)]])){
              metadata_speakers_comp_df17[u+endrow, "Mehrheit"]<-beschluss3[[length(beschluss3)]]$MEHRHEIT
            }
            if("ABSTIMMUNG_BEMERKUNG"%in%names(beschluss3[[length(beschluss3)]])){
              metadata_speakers_comp_df17[u+endrow, "Abstimmungsergebnis"]<-beschluss3[[length(beschluss3)]]$ABSTIMMUNG_BEMERKUNG
            }
          }
        }
        metadata_speakers_comp_df17[(1+endrow):(endrow + length(query3b)), "Reading"]<-3
        metadata_speakers_comp_df17[(1+endrow):(endrow + length(query3b)), "Comments"] <-paste0("Reading 3: Session:", plpr3[t])
        endrow<-nrow(metadata_speakers_comp_df17)
        total_speakers3<-total_speakers3+length(query3b)
      } else {
        metadata_speakers_comp_df17[endrow+1, "Comments"] <-paste0("No speakers in Reading 3: Session:", plpr3[t])
        metadata_speakers_comp_df17[endrow+1, "Reading"]<-3
        if(!is.null(xpathSApply(parsed_xml, query3.2a, xmlValue))){
          if("BESCHLUSS"%in%names(beschluss1[[length(beschluss3)]])){
            metadata_speakers_comp_df17[1+endrow, "Beschluss"]<-beschluss3[[length(beschluss3)]]$BESCHLUSS
          }
          if("BESCHLUSSSEITE"%in%names(beschluss1[[length(beschluss3)]])){
            metadata_speakers_comp_df17[1+endrow, "Beschlussseite"]<-beschluss3[[length(beschluss3)]]$BESCHLUSSSEITE
          }
          if("BESCHLUSSTENOR"%in%names(beschluss1[[length(beschluss3)]])){
            metadata_speakers_comp_df17[1+endrow, "Beschlusstenor"]<-beschluss3[[length(beschluss3)]]$BESCHLUSSTENOR
          }
          if("BEZUGSDOKUMENT"%in%names(beschluss1[[length(beschluss3)]])){
            metadata_speakers_comp_df17[1+endrow, "Beschlussdokument"]<-beschluss3[[length(beschluss3)]]$BEZUGSDOKUMENT
          }
          if("ABSTIMMUNGSART"%in%names(beschluss3[[length(beschluss3)]])){
            metadata_speakers_comp_df17[1+endrow, "Abstimmungsart"]<-beschluss3[[length(beschluss3)]]$ABSTIMMUNGSART
          }
          if("MEHRHEIT"%in%names(beschluss3[[length(beschluss3)]])){
            metadata_speakers_comp_df17[1+endrow, "Mehrheit"]<-beschluss3[[length(beschluss3)]]$MEHRHEIT
          }
          if("ABSTIMMUNG_BEMERKUNG"%in%names(beschluss3[[length(beschluss3)]])){
            metadata_speakers_comp_df17[1+endrow, "Abstimmungsergebnis"]<-beschluss3[[length(beschluss3)]]$ABSTIMMUNG_BEMERKUNG
          }
        }
        endrow<-nrow(metadata_speakers_comp_df17)
    ...
    metadata_speakers_comp_df17[endrow+1, "Comments"]<-"No 3rd debate"
    metadata_speakers_comp_df17[endrow+1, "Date_of_BT_debate3"]<-"No 3rd debate"
    metadata_speakers_comp_df17[endrow+1, "Reading"]<-3
    endrow<-nrow(metadata_speakers_comp_df17)
  ...
  #Inserting bill variables into the rows initialized by speakers (or lack thereof)
  metadata_speakers_comp_df17[(start:endrow),"Legislative_period"]<-wahl
  metadata_speakers_comp_df17[(start:endrow), "Static_URL"]<-url
  metadata_speakers_comp_df17[(start:endrow),"Sachgebiete"]<-sach_collapsed
  metadata_speakers_comp_df17[(start:endrow),"Title"]<-title
  metadata_speakers_comp_df17[(start:endrow),"Inhalt"]<-inhalt
  metadata_speakers_comp_df17[(start:endrow),"Initiator"]<-initiative_collapsed
  metadata_speakers_comp_df17[(start:endrow),"Gesta_number"]<-gesta
  metadata_speakers_comp_df17[(start:endrow),"Aktueller_Stand"]<-akt
  if (length(zust_proc)>0){
    if (str_detect(zust_proc, "Ja")) {metadata_speakers_comp_df17[(start:endrow),"Zustimmungspflicht_proclamation"]<-"yes"}
    if (str_detect(zust_proc, "Nein")) {metadata_speakers_comp_df17[(start:endrow),"Zustimmungspflicht_proclamation"]<-"no"}
  }
  if (length(zust_bill)>0) {
    if (str_detect(zust_bill, "Ja")) {metadata_speakers_comp_df17[(start:endrow),"Zustimmungspflicht_bill"]<-"yes"}
    if (str_detect(zust_bill, "Nein")) {metadata_speakers_comp_df17[(start:endrow),"Zustimmungspflicht_bill"]<-"no"}
  }
  if (length(drs_num)>0) { #if it exists, extract it
    metadata_speakers_comp_df17[(start:endrow),"Drucksachennummer"]<-drs_num  
  }
  metadata_speakers_comp_df17[(start:endrow),"Date_of_initiative"]<-init_date_value
  if (length(ver_dates)>0){
    ver_date_value<-str_extract(ver_dates, "[[:digit:]]([[:digit:]]|\\.){9}") #extracting the first date in this row
    metadata_speakers_comp_df17[(start:endrow),"Proclamation_date"]<-ver_date_value
  }
  if (length(inkraft)>0){
    inkraft<-inkraft[1] #selecting the first entry
    inkraft_value<-str_sub(inkraft, 1,10) #the date is the first 10 characters of the row
    metadata_speakers_comp_df17[(start:endrow),"Date_of_entry_into_force"]<-inkraft_value
  }
  #Plenary protocol and reading
  if (!is.null(plpr1)) {
    metadata_speakers_comp_df17[(start:endrow), "Plenary_protocol1"]<-paste(plpr1, collapse=";")
    debate1<-xpathSApply(parsed_xml, sprintf(("//URHEBER[contains(text(), '1. Beratung')]/following-sibling::FUNDSTELLE[contains(text(), '%s')]"), plpr1), xmlValue)
    debate1_date<-substr(debate1, 1,10) #extracting the date (1st 10 characters) from that line
    debate1_date<-unique(debate1_date)
    metadata_speakers_comp_df17[(start:endrow), "Date_of_BT_debate1"]<-paste(debate1_date, collapse=";")
  }
  if (!is.null(plpr2)) {
    metadata_speakers_comp_df17[(start:endrow), "Plenary_protocol2"]<-paste(plpr2, collapse=";")
    debate2<-xpathSApply(parsed_xml, sprintf(("//URHEBER[contains(text(), '2. Beratung')]/following-sibling::FUNDSTELLE[contains(text(), '%s')]"), plpr2), xmlValue)
    debate2_date<-substr(debate2, 1,10) #extracting the date (1st 10 characters) from that line
    debate2_date<-unique(debate2_date)
    metadata_speakers_comp_df17[(start:endrow), "Date_of_BT_debate2"]<-paste(debate2_date, collapse=";")
  }
  if (!is.null(plpr3)) {
    metadata_speakers_comp_df17[(start:endrow), "Plenary_protocol3"]<-paste(plpr3, collapse=";")
    debate3<-xpathSApply(parsed_xml, sprintf(("//URHEBER[contains(text(), '3. Beratung')]/following-sibling::FUNDSTELLE[contains(text(), '%s')]"), plpr3), xmlValue)
    debate3_date<-substr(debate3, 1,10) #extracting the date (1st 10 characters) from that line
    debate3_date<-unique(debate3_date)
    metadata_speakers_comp_df17[(start:endrow), "Date_of_BT_debate3"]<-paste(debate3_date, collapse=";")
  }
  endrow<-nrow(metadata_speakers_comp_df17)
}
for(i in nrow(metadata_speakers_comp_df17):1){
  if(is.na(metadata_speakers_comp_df17$Beschluss[i])){
    metadata_speakers_comp_df17$Beschluss[i]=metadata_speakers_comp_df17$Beschluss[i+1]
  }
}
metadata_speakers_comp_df17$Beschluss[is.na(metadata_speakers_comp_df17$`Speaker name`)]=NA
save(metadata_speakers_comp_df17, file="17leg_metadata_speakers_comp_df.Rdata")  

#load("17leg_metadata_speakers_comp_df.Rdata")
load("17leg_metadata_speakers_df_encoded.Rdata")

metadata_speakers_complete_df17<-rbind(metadata_speakers_df17, metadata_speakers_comp_df17) #combining the budget extraction with the regular extraction

save(metadata_speakers_complete_df17, file="17leg_metadata_speakers_complete_df_encoded.Rdata")
